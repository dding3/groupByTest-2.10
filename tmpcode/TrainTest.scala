...
val input2: Tensor[Double] = Tensor[Double](Storage[Double](Array(4.0, 5.0, 6.0, 7.0)))
      ...
      val batchSize = 2 * param.coreNumber
      ...
      val rdd = sc.get.parallelize(1 to 256 * 400000,
        partitionNum).coalesce(4, true).repartition(param.nodeNumber).map(prepareData).cache()
      rdd.count()
      ...
      val dataSet = new DistributedDataSet[MiniBatch[Double]] {
        ...
        override def size(): Long = 256 * 400000
