...
//put tag file under /home/ding/proj/InformationExtraction/ie/data/evaluation/manual
//put ori file under /home/ding/proj/InformationExtraction/ie/data/evaluation/ori_test
//then run tha app
object PrepareModel {
...
//    s.toString().replace("page-","").replace("_0_final.tsv","").replace("_"," ").split(",").foreach{x => print("\""+x+"\""+",")}
    println(s)

    //get all files with *.tsv, from *.txt to *.tsv
    val path = "/home/ding/proj/InformationExtraction/ie/data/evaluation/ori_test"
//    val path = "/home/ding/proj/InformationExtraction/ie/data/evaluation/test"
var fileNumber = 0
    new File(path).listFiles().foreach { folder =>
      ...
      folder.listFiles().foreach { file =>
        ...
        val tagPath = file.getParentFile.getParentFile.getParent + "/manual/" + file.getParent.split("/").last + "/" + file.getName
        if (new File(tagPath).exists()) {

println("file name: "+file.getName)
          Source.fromFile(tagPath).getLines.toList.foreach {line  =>
            val line2 = line.replaceAll("\u00a0", " ").replaceAll("\u200B|\u200C|\u200D|\uFEFF", "").replace("  ", " ")
            if (line2.contains("\t1")) {
              val nameStart = line2.indexOf("\t1") + 2
              var nameEnd = line2.indexOf("\t", nameStart)
              if(nameEnd == -1) nameEnd = line2.length
              val name = line2.substring(nameStart, nameEnd)
...
//              println("name:"+name)
              ...
              var di = 0
              while (line2.indexOf("\t3", di) != -1) {
                val departStart = line2.indexOf("\t3", di) + 2
                var departEnd = line2.indexOf("\t", departStart)
                if (departEnd == -1) departEnd = line2.length
                if (departEnd <= departStart) {
                  throw new RuntimeException(s"departments error: $company, $line2")
                }
                val department = (line2.substring(departStart, departEnd))
              ...
              var ti = 0
              while (line2.indexOf("\t2", ti) != -1) {
                val titleStart = line2.indexOf("\t2", ti) + 2
                var titleEnd = line2.indexOf("\t", titleStart)
                if (titleEnd == -1) titleEnd = line2.length
                if (titleEnd <= titleStart) {
                  throw new RuntimeException(s"title error: $company, $line2")
                }
                val title = (line2.substring(titleStart, titleEnd))
