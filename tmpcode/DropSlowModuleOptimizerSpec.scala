...
import com.intel.analytics.bigdl.parameters.{AllReduceParameter, AllReduceParameterManager, ImprovedAllReduceParameterManager, OneReduceParameterManager}
...
class DropSlowModuleOptimizerSpec extends FlatSpec with Matchers with BeforeAndAfter
  ...
  import DropSlowModuleOptimizerSpec.subModuleNumber
  ...
  it should "be same with grad agg optimizer" in {
    ...
    val test = optimizer.parameters.clone()
    mseWeight.fill(0.125)
    val pm2 = new AllReduceParameterManager[Double](mseWeight, dataSet2.partitions())
  ...
  it should "be same with grad agg optimizer with multi stack dataset" in {
    ...
    val test = optimizer.parameters.clone()
    mseWeight.fill(0.125)
    val pm2 = new AllReduceParameterManager[Double](mseWeight, dataSet.partitions())
    val optimizer2 = new GradAggEpochOptimizer[Double](mseModule, MSECriterion[Double],
  ...
  "An Artificial Neural Network with Cross Entropy and LBFGS" should
    ...
    val optimizer = new DropSlowModuleGradAggEpochOptimizer[Double](crnModule, ClassNLLCriterion[Double](),
      new LBFGS, pm, dataSet, new Metrics)
  ...
  it should "be same with grad agg optimizer" in {
    ...
    val test = optimizer.parameters
    crnWeight.fill(0.125)
    val pm2 = new AllReduceParameterManager[Double](crnWeight, dataSet2.partitions())
