...
class LosslessCompressedTensor[T: ClassTag](buffer: Array[Byte],
  bufferOffset: Int, bufferLength: Int)
  extends CompressedTensor[T] {

  def this(tensor: Tensor[T]) {
    this(new Array[Byte](tensor.nElement() * 8), 0, tensor.nElement() * 8)
    compress(tensor)
  }

  def this(length: Int) = this(new Array[Byte](length * 8), 0, length * 8)
  ...
  : this.type = {
    ...
    if (classTag[T] == classTag[Double]) {
      LosslessCompressedTensor.toFP16(src.storage().array().asInstanceOf[Array[Double]], tOffset,
    ...
    System.arraycopy(src.storage().array(), tOffset, buffer, bufferOffset + offset, length)
...
object LosslessCompressedTensor {
...
//        var i = start
//        while (i < end) {
//          val bytes = src(i + srcOffset).toByte
//          tgt(tgtOffset + i * 2) = (bytes >>> 24 & 0xff).toByte
//          tgt(tgtOffset + i * 2 + 1) = (bytes >>> 16 & 0xff).toByte
//          i += 1
//        }
        val nioBuf = java.nio.ByteBuffer.allocate(8*(end-start))
        val buf = nioBuf.asDoubleBuffer.put(src, start, end-start).array()
        System.arraycopy(buf, 0, tgt, tgtOffset, buf.length)
      })
    )
  ...
  private[parameters] def fromFP16(fp16: Array[Byte], fp16Offset: Int, fp16Length: Int,
    ...
    Engine.default.invokeAndWait(
      (0 until availableTask).map(tid => () => {
        ...
        val nioBuf = java.nio.DoubleBuffer.allocate((end-start)/8)
        nioBuf.put(java.nio.ByteBuffer.wrap(fp16).asDoubleBuffer())
        val a = dbuf.array
        val dbuf = java.nio.DoubleBuffer.allocate(b.length/8)
        dbuf.put(java.nio.ByteBuffer.wrap(b).asDoubleBuffer)
        val a = dbuf.array
